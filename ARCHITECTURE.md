...
- **One repeating 7-day cycle**, not a calendar of specific dates — still true of any single active file. **Multi-week rotation is now confirmed real** (Week 2 replaced Week 1 exactly 7 days after Week 1's effective date — `PRD.md` §11), but not yet automated: each new week is still a manual single-file replace, the same `load_active_menu()` "most recently effective file" logic as before. Automating it (multiple files + a `week` lookup keyed off days-since-anchor) is the natural extension once the cycle length is certain — deliberately not built from just two data points.
...
1. User supplies the new menu — a poster photo, or (as happened for the Week 2 update) a structured JSON file they've already prepared.
2. User asks Claude (in this project) to swap it in.
3. **Claude normalizes it to the canonical schema first** — a photo needs full extraction; a user-supplied JSON still needs its shape checked against `meals.<meal>.items.<day>` (§4), since it hasn't consistently matched (the Week 2 file used flat `breakfast`/`lunch`/... top-level keys with a `days` sub-key, not `meals`/`items`). Never assume a new file's shape matches without checking.
4. **Check for new `(Non-Veg)`-tagged items against `strip_nonveg()`'s assumptions** (§13) before shipping — a new file can use a different tagging convention than the last one, as Week 2 did. Enumerate every `(Non-Veg)` occurrence and manually verify the filter's output for each, the way Week 2's swap did, rather than assuming the existing logic generalizes.
5. Claude replaces (or adds) the file in `data/menus/`. Whether to replace the single active file or add a second one alongside it depends on whether multi-week rotation has been built yet (§4, §11 in `PRD.md`) — currently unbuilt, so it's always a straight replace.
6. Commit + push. Next tick picks up the new data automatically — no script/workflow changes needed for a pure menu content update (schema conversion and filter-assumption checks in steps 3–4 are data prep, not code changes).
...
- **`strip_nonveg(item)`:** the filtering logic itself. The source menu data combines veg/non-veg alternatives into one string per item, e.g. `"Paneer Chatpata (Veg), Chicken Kosha (Non-Veg)"`, in no consistent order or tagging convention. The function splits on whichever separator is used (`" / "` or `", "`) and **keeps whichever half does *not* carry `"(Non-Veg)"`**, then strips a `"(Veg)"` tag from what's kept *if one is present*. An item with no `"(Non-Veg)"` marker at all (the majority — plain vegetarian dishes) passes through untouched.
  - **Revised 2026-09-21** after the Week 2 menu swap exposed a gap: the original version required the kept half to explicitly contain `"(Veg)"` to be recognized — true for every combined item in Week 1's data, but Week 2 tags *only* the non-veg half (e.g. `"Veg Kolapuri / Kolhapuri (Non-Veg)"` — the veg half has no parenthetical tag, just the word "Veg" as part of the dish name). The old logic would have silently stopped filtering the moment this data went live. Caught before shipping by enumerating every `(Non-Veg)`-tagged item in the new file first (see `MEMORY.md`) — re-verified against both weeks' combined items plus plain veg items before the swap.
