...
**Fix:** cron-job.org (free external scheduler) now calls the `workflow_dispatch` REST API every 5 minutes instead of relying on GitHub's internal `schedule` trigger (kept only as a fallback). This is the fix GitHub's own community discussions recommend for anything needing real timing precision. Setup requires a narrowly-scoped GitHub PAT (Actions: write only, this repo only) — a manual step only the repo owner can do; not yet completed as of this writing (see `PLAN.md` Phase 1.6).
